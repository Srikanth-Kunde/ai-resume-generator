  ...
  return (
    <div className="bg-white p-12 mx-auto font-serif text-slate-800 shadow-sm" style={{ ...getThemeStyle(data.themeColor), minHeight: '297mm', width: '100%', maxWidth: '210mm', fontSize: '11.5px', lineHeight: '1.6' }}>
      <header style={{ textAlign: 'center', marginBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '20px', marginTop: '20px', color: '#64748b', fontSize: '11px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '35px' }}>
        ...
        {data.projects.some(p => p.name) && (
          <section>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
              <div style={{ flex: 1, height: '1px', background: 'var(--color-premium-200, #e2e8f0)' }} />
              <h2 style={{ fontSize: '14px', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', letterSpacing: '2px' }}>Projects</h2>
              <div style={{ flex: 1, height: '1px', background: 'var(--color-premium-200, #e2e8f0)' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {data.projects.filter(p => p.name).map(proj => (
                <div key={proj.id} style={{ textAlign: 'center' }}>
                  <h3 style={{ fontSize: '15px', fontWeight: 900, color: '#0f172a', margin: '0 0 4px' }}>{proj.name}</h3>
                  {proj.technologies && <p style={{ fontSize: '10px', color: 'var(--color-premium-500, #64748b)', margin: '4px 0' }}>{proj.technologies}</p>}
                  <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '4px', alignItems: 'center' }}>
                    {generateBulletPoints(proj.description).map((b, i) => (
                      <li key={i} style={{ fontSize: '11.5px', color: '#334155', maxWidth: '600px' }}>{b}</li>
                    ))}
                  </ul>
                </div>
              ))}
          ...
          <div>
            {data.certifications.some(c => c.name) && (
              <section style={{ marginBottom: '30px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '15px' }}>
                  <div style={{ flex: 1, height: '1px', background: 'var(--color-premium-200, #e2e8f0)' }} />
                  <h2 style={{ fontSize: '13px', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', letterSpacing: '2px' }}>Certs</h2>
                  <div style={{ flex: 1, height: '1px', background: 'var(--color-premium-200, #e2e8f0)' }} />
                </div>
                {data.certifications.filter(c => c.name).map(cert => (
                  <div key={cert.id} style={{ textAlign: 'center', marginBottom: '10px' }}>
                    <h4 style={{ fontSize: '11.5px', fontWeight: 900, color: '#0f172a' }}>{cert.name}</h4>
            ...
            {achievements.length > 0 && (
              <section>
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '15px' }}>
                  <div style={{ flex: 1, height: '1px', background: 'var(--color-premium-200, #e2e8f0)' }} />
                  <h2 style={{ fontSize: '13px', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', letterSpacing: '2px' }}>Achievements</h2>
                  <div style={{ flex: 1, height: '1px', background: 'var(--color-premium-200, #e2e8f0)' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'center' }}>
                  {achievements.map((a, i) => (
                    <div key={i} style={{ fontSize: '11px', color: '#334155', textAlign: 'center' }}>
                      {a}
                    </div>
                  ))}
                </div>
